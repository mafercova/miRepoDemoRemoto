...
ipcMain.handle('asistencias:listar', async (_, { grupo_id, fecha }) => {
  const [rows] = await pool.query(`
    SELECT a.id, a.nombre,
      COALESCE(asist.presente, 0) AS presente,
      asist.fecha
    FROM alumnos a
...
ipcMain.handle('asistencias:dashboard', async (_, { grupo_id }) => {
  ...
  const totalAlumnos = total[0][0].c;

  const [rows] = await pool.query(`
    SELECT
      a.id, a.nombre,
      COUNT(asist.alumno_id) AS total_clases,
      SUM(asist.presente) AS presentes,
  ...
  const fechas = await pool.query(`
    ...
    WHERE alumno_id IN (SELECT id FROM alumnos WHERE grupo_id = ?)
    ORDER BY fecha
  `, [grupo_id]);

  return { alumnos: rows, totalAlumnos, fechas: fechas[0] };
});
